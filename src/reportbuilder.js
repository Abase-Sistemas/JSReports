...
/*
 * Ditto 1.4.122
 * Copyright (c) 2017 Ditto
 * http://dittojs.com
 */

/**
 * Do not call directly.  Use <code>ditto.createReport()</code> to get a new ReportBuilder.
 * @class ReportBuilder
 * @classdesc Provides an API for programmatic construction of report definitions.  Use <code>ditto.createReport()</code>
...
ditto.ReportBuilder = (function() {
    var nextReportId = 1;
    var version = "1.4.122";
    return function(opts) {
        if (!ditto.defaultReport) {
            throw new Error('Missing Ditto base library - did you include all required files?');
        }
        this.def = ditto.merge({}, ditto.defaultReport, opts);
...
const getDefaultDataSection = () => JSON.parse(JSON.stringify(ditto.defaultReport.body));

ditto.merge(ditto.ReportBuilder.prototype, function() {
    ...
    var api = /** @lends ReportBuilder.prototype */ {
        ...
        input: function(name, type, defaultVal, displayName, opts) {
            this.def.inputs.push(ditto.merge({
                name: name,
                type: type,
        ...
        text: function(text, left, top, width, height, opts = {}) {
            if (opts.font && typeof opts.font === 'string') {
                opts.font = { name: opts.font, css: opts.font };
            }
            this.lastSection.elements.push(ditto.merge(ElementUtils.getDefaultDef('text'), {
        ...
        image: function(url, left, top, width, height, opts) {
            this.lastSection.elements.push(ditto.merge(ElementUtils.getDefaultDef('image'), {
                id: getNextControlId.call(this),
                url: url,
                left: left || 0,
                top: top || 0,
        ...
        barcode: function(value, encoding, left, top, width, height, opts) {
            this.lastSection.elements.push(ditto.merge({
                id: getNextControlId.call(this),
                type: "barcode",
        ...
        checkbox: function(left, top, width, height, checked, opts) {
            this.lastSection.elements.push(ditto.merge({
                id: getNextControlId.call(this),
                type: 'checkbox',
                left: left || 0,
        ...
        chart: function(chartType, valueField, labelField, left, top, width, height, opts) {
            ...
            this.lastChart = ditto.merge(template, opts);
        ...
        column: function(width, detail, header, footer, opts) {
            ...
            this.lastTable.columns.push(ditto.merge({
                width: width,
                detail: detail,
                header: header || null,
                footer: footer || null
        ...
        subLevel: function(dataSource, joinParentField, joinChildField, groupBy, sortBy, sortDir) {
            if (typeof sortDir !== "undefined" && sortDir !== "asc" && sortDir !== "desc") {
                throw new Error('Allowed values for groupBy sortDir argument are: "asc", "desc"');
            }
            var default_section_height = getDefaultSectionHeight.call(this);
            var group = {
                ...
                sort_by: sortBy || groupBy || null,
                sort_dir: sortDir || "asc",
                where_parent_field: joinParentField || null,
                where_child_field: joinChildField || null,
                header: {
                    visible: false,
                    height: default_section_height,
                    elements: []
         ...
         * @instance
         */
        element: function(type, left, top, width, height, opts) {
            if (typeof type !== 'string') throw new Error(`element() method expected element type ID as first argument; got ${typeof type} ${JSON.stringify(type)} instead.`);
            this.lastSection.elements.push(ditto.merge({
                id: getNextControlId.call(this),
                type: type,
                left: left || 0,
 ...
 *    .done();
 */
ditto.createReport = function(opts) {
    // Single string argument taken to be report name
    if (typeof opts === 'string') {
        opts = { name: opts };
    }
    return new ditto.ReportBuilder(opts);
};
